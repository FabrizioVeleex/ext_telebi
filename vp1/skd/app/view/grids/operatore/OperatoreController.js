 ...
 */
Ext.define('skd.view.grids.operatore.OperatoreController', {
    extend: 'skd.view.grids.GridsController',
    alias: 'controller.operatore',
    requires:[
        'skd.store.grids.operatore.Operatore',
        'skd.view.grids.operatore.GridOperatore'
    ],
    ...
    * ---------------------------------------------------------------------------------------------------------*/
    onPrepareStoreData:function (rest,idgrid) {
        ...
        this.storeDataIn = rest.data.store['operatore'];
        this.columnDataIn = rest.data.columns['operatore'];
    ...
    * ---------------------------------------------------------------------------------------------------------*/
    onGenerateGridStore:function () {
        ...
        this.groupingF = Ext.create('Ext.grid.feature.Grouping', {
            ...
            showSummaryRow:true,
            groupHeaderTpl: '{columnName}: <b>{name}</b> <i>{[values.children[0].data["priority_description"]]}</i> ({rows.length} Part{[values.rows.length > 1 ? "s" : ""]})',
            hideGroupedHeader: false,
            enableGroupingMenu: false
        });

        this.store = Ext.create('skd.store.grids.operatore.Operatore',{
        ...
        this.grid = Ext.create('skd.view.grids.operatore.GridOperatore',{
            features: [
                this.groupingF
            ],
