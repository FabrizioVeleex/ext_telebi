 ...
 */
Ext.define('gnc.view.forms.scheda.grids.GridFormazione', {
    ...
    requires: [
        ...
        'gnc.store.forms.scheda.ComboFormazione',
    ...
    columns: [
        {xtype: 'actioncolumn', menuDisabled:true, resizable:false, sortable:false, width: 30,
            items: [{
                getClass: function( view, meta, record){
                    ...
                    if (vm.get('readOnlyMan') || record.data.tipologia==='') {
                ...
                handler: function (view, rowIndex, colIndex, item, event, record) {
                    ...
                    if (record.get('action') === 2) {
                        ...
                        if (record.data.isnew===0){
                            ...
                            rec.data.formazione = rec.data.formazione.filter(obj=>obj.id!==record.data.id) //lo rimuovo dal record backend
        ...
        {text: Locale.t('gnc.forms.scheda.grids.formazione.tipologia'), dataIndex: 'tipologia',flex:1,
            getEditor: function (record) {
                let vm = this.lookupViewModel()
                if (vm.get('readOnlyMan') === true || record.data.isnew===0) {
                    return false
                }
                let store = Ext.create('gnc.store.forms.scheda.ComboFormazione');
                return {
                    ...
                    tpl: Ext.create('Ext.XTemplate',
                        ...
                        '<li role="option" class="x-boundlist-item" style="border-bottom: 1px solid black"><b>Corso</b>: {numero} - {tipologia}<br><b>Data</b>: {datac} - <b>Scadenza</b>: {datasca}<br><b>Sede</b>: {sede}</li>',
                        '</tpl></ul>'
                    ),
                    listeners: {
                        select: function (cmb,record) {
                            ...
                            row.data['idcorso'] = record.data['id'];
                            ...
                            if (lastrecord === row) {
                                rec.data.formazione.push(row.data) //lo metto nel record backend
        ...
        {text: Locale.t('gnc.forms.scheda.grids.formazione.numero'), dataIndex: 'numero', width: 100},
        {text: Locale.t('gnc.forms.scheda.grids.formazione.datac'), dataIndex: 'datac', width: 100},
        {text: Locale.t('gnc.forms.scheda.grids.formazione.datasca'), dataIndex: 'datasca', width: 100},
        {text: Locale.t('gnc.forms.scheda.grids.formazione.sede'), dataIndex: 'sede',width:200}
    ]
});