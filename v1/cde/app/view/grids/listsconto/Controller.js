 ...
 */
Ext.define('cde.view.grids.listsconto.Controller', {
    extend: 'portal.v1.view.grids.DefaultController',
    mixins: ['portal.v1.global.Util'],
    alias: 'controller.v1-cde-listsconto',
    requires: [
        'Ext.button.Button',
        ...
        'Ext.window.Window',
        'cde.view.forms.listsconto.Panel'
    ],
    init: function () {
    ...
    checkColumn: function(griglia) {
        ...
        if (this.checkRuoli(['99','1'])){
            if (nodo.idpadre==='-1' || nodo.idpadre==='0') {
                this.toolbar.add({
                    tooltip: Locale.t('cde.grids.listsconto.btn.sitook.tooltip'),
                    text: Locale.t('cde.grids.listsconto.btn.sitook.text'),
                    msg: Locale.t('cde.grids.listsconto.btn.sitook.msg'),
            ...
            if (nodo.idpadre==='1' || nodo.idpadre==='0') {
                this.toolbar.add({
                    tooltip: Locale.t('cde.grids.listsconto.btn.sitoko.tooltip'),
                    text: Locale.t('cde.grids.listsconto.btn.sitoko.text'),
                    msg: Locale.t('cde.grids.listsconto.btn.sitoko.msg'),
    ...
    createForm: function (view, record, isnew) {
        ...
        this.getView().fireEvent('createTab',Ext.create('cde.view.forms.listsconto.Panel', {
    ...
    onafterrendergrid: function (grid) {
        grid.myColumns =[
            ...
            {text: Locale.t('cde.grids.listsconto.column.cd_gruppo'), dataIndex: 'cd_gruppo', width:100,filter: {type: 'string'}},
            {text: Locale.t('cde.grids.listsconto.column.cliente'), dataIndex: 'cliente', flex:1,filter: {type: 'string'}},
            {text: Locale.t('cde.grids.listsconto.column.dateins'), dataIndex: 'dateins',width:150,xtype: 'datecolumn', format: 'd/m/Y', filter: {type: 'date',dateFormat: 'Ymd'}},
            {text: Locale.t('cde.grids.listsconto.column.datestart'), dataIndex: 'datestart',width:150,xtype: 'datecolumn', format: 'd/m/Y', filter: {type: 'date',dateFormat: 'Ymd'}},
            {text: Locale.t('cde.grids.listsconto.column.sconto1'), dataIndex: 'sconto1', width:100,filter: {type: 'number'}},
            {text: Locale.t('cde.grids.listsconto.column.sconto2'), dataIndex: 'sconto2', width:100,filter: {type: 'number'}}
        ]
        this.callParent(arguments)
    },
    //azioni trasmissione
    onAzione:function(btn) {
        ...
        if (recordsGood.length===0) {
            Ext.Msg.show({
                ...
                msg: Locale.t('cde.grids.listsconto.obbrec'),
                buttons: Ext.Msg.OK,
        ...
        let btnConfirm = new Ext.Button({
            text: Locale.t('global.btn.conferma'), iconCls: 'x-fas fa-check', handler: function () {
                ...
                Ext.Ajax.request({
                    method: 'PUT',
                    jsonData:{data:recordsGood,azione:btn.azione,listino:'sconti'}, //passo anche listino riferimento
