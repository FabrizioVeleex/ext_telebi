Ext.define("orf.forms.documento.controller.Controller", {
  ...
  requires: [
    "Ext.window.Toast",
    "orf.forms.documento.Model",
    "orf.forms.documento.controller.ManagerBtn",
    "orf.forms.documento.controller.ManagerCards",
    'Ext.drag.Target',
  ...
  init: function () {
    ...
    vm.set(
      "record",
      Ext.create("orf.forms.documento.Model", {
  ...
  managerView: function () {
    ...
    let me = this,
      ...
      managerBtn = Ext.create("orf.forms.documento.controller.ManagerBtn"),
      managerCards = Ext.create("orf.forms.documento.controller.ManagerCards");

    try {
      // Titolo tab
      vm.set("title", record.data["num_doc"] + " - " + record.data["rag_soc"] || "n.d.");
      vm.set("label", Locale.t("orf.forms.documento.title"));
  ...
  onScarta: function () {
    ...
    Ext.Msg.show({
      title: Locale.t('orf.forms.documento.btn.scarta.text'),
      iconCls: 'x-fas fa-trash',
      message: Locale.t('orf.forms.documento.btn.scarta.message'),
  ...
  onDwnExcel: function () {
    ...
    Ext.Ajax.request({
      ...
      failure: function (response) {
        ...
        // let errore = Locale.t("orf.forms.documento.errore") + ": " + response.statusText;
  ...
  onShowPdf: function (pnl) {
    ...
    Ext.Ajax.request({
      ...
      binary: true,
      jsonData: {
        record: { ...record.data, firma: pnl.posizione, tipodoc: "ORF" },
      },
      success: function (response) {
        let headers = response.getAllResponseHeaders();
        let blob = new Blob([response.responseBytes], { type: headers["content-type"] });
  ...
  onShowPdfDup: function (pnl) {
    ...
    Ext.Ajax.request({
      ...
      binary: true,
      jsonData: {
        record: { ...record.data, id_duplicato: pnl.posizione, tipodoc: "ORF" },
      },
      success: function (response) {
        let headers = response.getAllResponseHeaders();
        let blob = new Blob([response.responseBytes], { type: headers["content-type"] });
  ...
  onErrorShowPdf: function (pnl) {
    ...
    if (cardtmp.length === 1) {
        ...
        items: [
          {
            ...
            flex: 1,
            bind: {
              html: Locale.t("orf.forms.documento.docnotfound"),
            },
          },
      ...
      Ext.Msg.show({
        title: Locale.t('global.attenzione'),
        msg: Locale.t("orf.forms.documento.docnotfound"),
        buttons: Ext.Msg.OK,
        icon: Ext.MessageBox.ERROR
      });
  ...
  onSendNewEmail: function () {
    ...
    if (!mailto) {
      Ext.Msg.show({
        title: Locale.t("global.attenzione"),
        message: Locale.t("global.form.validation.form") + " " + Locale.t("orf.forms.documento.fields.mailto.label"),
        buttons: Ext.Msg.OK,
        icon: Ext.MessageBox.ERROR,
      });
      return false;
    ...
    Ext.Ajax.request({
      ...
      success: function (res) {
        ...
        Ext.toast({
          html: Locale.t("orf.forms.documento.sendmail.successsend"),
          closable: false,
          align: 't',
          slideInDuration: 400,
          minWidth: 400
      ...
      failure: function () {
        ...
        Ext.Msg.show({
          title: Locale.t("global.attenzione"),
          message: Locale.t("orf.forms.documento.sendmail.errorsend"),
          buttons: Ext.Msg.OK,
          icon: Ext.MessageBox.ERROR,
        });
      }
  ...
  onDragEnter: function () {
    const b = this.getView().down("#v1-orf-dragattach");
  ...
  onDrop: function (target, info) {
    let me = this,
      ...
      panelDrop = view.down("#v1-orf-dragattach"),
      tagAttach = view.down('v1-orf-tagattach-combo'),
      files, len, s;
    files = info.files;
    len = files.length;
    ...
    oReq.addEventListener("loadend", function (e) {
      if (e.currentTarget.status === 200) {
        ...
        s = Locale.t("orf.forms.documento.errors.fileupload");
      }
    ...
    oReq.addEventListener("error", function () {
      Ext.toast({
        html: Locale.t("orf.forms.documento.errors.fileuploadsm"),
        closable: false,
        align: 't',
        slideInDuration: 400,
        minWidth: 400
  ...
  onBuildPdf: function () {
    ...
    Ext.Ajax.request({
      ...
      success: function () {
        Ext.toast({
          html: Locale.t("orf.forms.documento.pdf.regenerationstart"),
          closable: false,
          align: 't',
          slideInDuration: 400,
          minWidth: 400
      ...
      failure: function () {
        Ext.toast({
          html: Locale.t("orf.forms.documento.pdf.regenerationerror"),
          closable: false,
          align: 't',
          slideInDuration: 400,
          minWidth: 400
  ...
  runDupOpen: function (record) {
    ...
    let newTab = {
      ...
      text: Locale.t("orf.forms.documento.infodoc"),
      btn: {
        xtype: "splitbutton",
