 ...
 */
Ext.define("orf.global.firma.ViewModel", {
    ...
    previewstampante: {
      ...
      get: function (data) {
        if (data.copie > 0) {
          return (
            "<h2>" +
            Locale.t("orf.grids.documenti.winfirma.selezionata") +
            ": " +
            data.nomestampante +
            "," +
            Locale.t("orf.grids.documenti.winfirma.numcopie") +
            ": " +
            data.copie +
            "</h2>"
          );
        } else {
          return "<h2>" + Locale.t("orf.grids.documenti.winfirma.nostampante") + "</h2>";
        }
      },
    },
    btnfirma: {
    ...
    btnfirmatips: {
      ...
      get: function (data) {
        if (!data.conducente && !data.vettore && !data.cessionario) {
          return Locale.t("orf.grids.documenti.winfirma.btn.avvia.tipsdisable");
        }
        return Locale.t("orf.grids.documenti.winfirma.btn.avvia.tipsenable");
      },
    },
  },
});
