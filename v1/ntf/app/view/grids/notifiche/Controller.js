 ...
 */
Ext.define('ntf.view.grids.notifiche.Controller', {
    extend: 'portal.v1.view.grids.DefaultController',
    alias: 'controller.notifiche',
    ...
    checkColumn: function(griglia) {
        ...
        if (this.toolbar) {
            ...
            if (this.checkRuoli(['99', '10'])) {
                if (nodo['stato']==='N') {
                    this.toolbar.add({
                        tooltip: Locale.t('ntf.grids.azioni.btn.tooltip'),
                        text: Locale.t('ntf.grids.azioni.btn.text'),
                        ui: 'ocra',
    ...
    onPress: function(btn) {
        ...
        grid.getPlugin('notifichePreview').toggleExpanded(btn.pressed);
    },
    ...
    onRead: function() {
        ...
        if (recordsGood.length===0) {
            Ext.Msg.show({
                ...
                msg: Locale.t('ntf.grids.azioni.btn.errore'),
                buttons: Ext.Msg.OK,
        ...
        let btnConfirm = new Ext.Button({
            text: Locale.t('global.btn.conferma'), iconCls: 'x-fas fa-check', handler: function () {
                ...
                Ext.Ajax.request({
                    ...
                    url: Backend.REST_API + 'grids/impostaeseguiti',
                    success: function () {
        ...
        let wdwpanel = Ext.create('Ext.form.Panel', {
            border: false, items: [
                {xtype: 'container',
                    style: {'padding': '5px'},
                    html: '<span style="font-weight:bold;color:red;font-size:12px;" >'+Locale.t('ntf.grids.azioni.btn.messaggio')+'</span>'
                }
            ]
        });
        let wndw = Ext.create('Ext.Window', {
            tbar: [btnX, btnConfirm], title: Locale.t('ntf.grids.azioni.btn.text'),
