 ...
 */
Ext.define("spl.global.component.griddocumenti.GridDocumentiFirma", {
  extend: "Ext.grid.Panel",
  alias: "widget.griddocumenti_firma",
  ...
  columns: [
      ...
      items: [
        {
          getClass: function (view, metadata, record) {
            if (record.get('status_spool') === 0) {
              metadata.tdAttr = `data-qtip="${Locale.t('spl.grids.documenti.winfirma.nospool')}"`;
              return 'x-fas fa-times bd-action-null bd-color-red';
            }
            metadata.tdAttr = `data-qtip="Documento pronto per la firma "`; //TODO${Locale.t('spl.grids.documenti.winfirma.nospool')}
    ...
    },
    {
      ...
      menuDisabled: true,
      resizable: false,
      dataIndex: "check_sogg",
      items: [
        {
          getClass: function (view, metadata, record) {

            if (record.get('cd_sogg_fat') === "") {
              metadata.tdAttr = `data-qtip="Manca l'associazione ad un soggetto, associare prima di procedere con la firma"`;
              return 'x-fas fa-times bd-action-null bd-color-red';
            }
            metadata.tdAttr = `data-qtip="Soggetto di apparteneza presente"`;
            return 'x-fas fa-check bd-action-null bd-color-green';
          },
        }
      ],
