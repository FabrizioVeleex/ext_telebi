 ...
 */
Ext.define('amm.view.forms.modulowidget.Controller', {
    extend: 'portal.v1.view.forms.mainCard.Controller',
    mixins: [
        'portal.v1.global.Util',
    ],
    alias: 'controller.v1-modulowidget',
    ...
    requires: [
        ...
        'amm.model.forms.modulowidget.Model',
        'amm.view.forms.modulowidget.cards.GridAutorizzazioni',
        'amm.view.forms.modulowidget.cards.GridRuoli',
        'amm.view.forms.modulowidget.cards.GridVersioni',
        'amm.view.forms.modulowidget.cards.Info'
    ],
    ...
    init: function() {
        ...
        vm.set('record', Ext.create('amm.model.forms.modulowidget.Model', {
    ...
    managerView: function () {
        ...
        try {
            ...
            vm.set('label', Locale.t('amm.forms.modulowidget.title'))
            //cards
            if (!this.listForms) {
                this.listForms = [
                    {posizione: 'info', backgroundColor: 'LightBlue',
                        card: Ext.create('amm.view.forms.modulowidget.cards.Info'),
                        text: Locale.t('amm.forms.modulowidget.info')
                    },
                    {posizione: 'autorizzazioni', backgroundColor: '',
                        card: Ext.create('amm.view.forms.modulowidget.cards.GridAutorizzazioni'),
                        text: Locale.t('amm.forms.modulowidget.autorizzazioni')
                    },
                    {posizione: 'ruoli', backgroundColor: '',
                        card: Ext.create('amm.view.forms.modulowidget.cards.GridRuoli'),
                        text: Locale.t('amm.forms.modulowidget.ruoli')
                    },
                    {posizione: 'versioni', backgroundColor: '',
                        card: Ext.create('amm.view.forms.modulowidget.cards.GridVersioni'),
                        text: Locale.t('amm.forms.modulowidget.version')
                    }
                ]
            }
            //Aggiungo cards
        ...
        storeruoli.each(function (rec) {
            if (rec.data.ruolo!=='') {
                if (rec.data.valore!=='' && !Ext.Array.contains(loadRuo, rec.data.ruolo)) {
                    ...
                    errRuoli = Locale.t('amm.forms.modulowidget.errori.erraccesso')
                }
            }
        })
        if (errRuoli!=='') {
        ...
        storeautorizzazioni.each(function (rec) {
            if (rec.data.idrisorsa!=='') {
                if (rec.data.idruoloass!=='' && !Ext.Array.contains(loadAut, rec.data.idrisorsa)) {
                    ...
                    errAutoriz = Locale.t('amm.forms.modulowidget.errori.errautorizzazioni')
                }
            }
        })
        if (errAutoriz!=='') {
        ...
        let formVersion = Ext.create('Ext.form.Panel',{
            ...
            dockedItems: {
                xtype: 'toolbar', dock: 'top', items: [
                    {
                        socpe:this,
                        xtype: 'button', text: Locale.t('amm.forms.modulowidget.gridversion.btn.annulla'),
                        iconCls: 'x-fas fa-times-circle',
                        ui: 'ocra',
                        handler: function(btn){this.up('window').fireEvent('closeWinVer',btn)}
                    },{
                        xtype: 'button', text: Locale.t('amm.forms.modulowidget.gridversion.btn.conferma'),
                        iconCls: 'x-fas fa-times-circle',
                        ui: 'green',
            ...
            items:[
                {xtype:'textfield',fieldLabel: Locale.t('amm.forms.modulowidget.gridversion.versione'),width:180,value:record.get('ver')},
                {xtype:'datefield',fieldLabel: Locale.t('amm.forms.modulowidget.gridversion.data'),width:250,format: 'd/m/Y',value:record.get('dataver')},
                {xtype:'displayfield',labelWidth:250,fieldLabel: Locale.t('amm.forms.modulowidget.gridversion.note')},
                {xtype:'htmleditor',width:720,value:record.get('note')}
            ]
        });
        let winVersion = Ext.create('Ext.Window',{
            title:Locale.t('amm.forms.modulowidget.gridversion.titolo'),
            layout:'fit',
            items:[
                formVersion
            ]
    ...
    onEditWinVer:function (btn) {
        ...
        if (ver.trim()==='' || !data){
            Ext.Msg.show({
                title: Locale.t('global.attenzione'),
                msg: Locale.t('amm.forms.modulowidget.gridversion.obb'),
                buttons: Ext.Msg.OK,
                icon: Ext.MessageBox.ERROR
            });
            return
