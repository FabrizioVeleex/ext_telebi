Ext.define("pak.forms.documento.controller.Controller", {
  ...
  requires: [
    "Ext.window.Toast",
    "pak.forms.documento.Model",
    "pak.forms.documento.controller.ManagerBtn",
    "pak.forms.documento.controller.ManagerCards",
    'Ext.drag.Target',
  ...
  init: function () {
    ...
    vm.set(
      "record",
      Ext.create("pak.forms.documento.Model", {
  ...
  managerView: function () {
    ...
    let me = this,
      ...
      managerBtn = Ext.create("pak.forms.documento.controller.ManagerBtn"),
      managerCards = Ext.create("pak.forms.documento.controller.ManagerCards");

    try {
      // Titolo tab
      vm.set("title", record.data["num_doc"] + " - " + record.data["rag_soc"] || "n.d.");
      vm.set("label", Locale.t("pak.forms.documento.title"));
  ...
  onScarta: function () {
    ...
    Ext.Msg.show({
      title: Locale.t('pak.form.documento.btn.scarta.text'),
      iconCls: 'x-fas fa-trash',
      message: Locale.t('pak.form.documento.btn.scarta.messaggio'),
  ...
  onDwnExcel: function () {
    ...
    Ext.Ajax.request({
      ...
      failure: function (response) {
        ...
        // let errore = Locale.t("pak.forms.documento.errore") + ": " + response.statusText;
  ...
  onShowPdfDup: function (pnl) {
    ...
    Ext.Ajax.request({
      ...
      binary: true,
      jsonData: {
        record: { ...record.data, id_duplicato: pnl.posizione, tipodoc: "BOL" },
      },
      success: function (response) {
        let headers = response.getAllResponseHeaders();
        let blob = new Blob([response.responseBytes], { type: headers["content-type"] });
  ...
  onSendNewEmail: function () {
    ...
    // const mailto = email.mailto && email.mailto.length > 0 ? email.mailto : false; //OLD
    ...
    if (!mailto) {
      Ext.Msg.show({
        title: Locale.t("global.attenzione"),
        message: Locale.t("global.form.validation.form") + " " + Locale.t("pak.forms.documento.fields.mailto"),
        buttons: Ext.Msg.OK,
        icon: Ext.MessageBox.ERROR,
      });
      return false;
    ...
    Ext.Ajax.request({
      ...
      success: function (res) {
        ...
        Ext.toast({
          html: Locale.t("pak.forms.documento.sendmail.successsend"),
          closable: false,
          slideInDuration: 400,
          minWidth: 400
        });
      ...
      failure: function () {
        ...
        Ext.Msg.show({
          title: Locale.t("global.attenzione"),
          message: Locale.t("pak.forms.documento.sendmail.errorsend"),
          buttons: Ext.Msg.OK,
          icon: Ext.MessageBox.ERROR,
        });
      }
  ...
  afterRenderNewEmail: function (view) {
    ...
    if (window.File && window.FileList && window.FileReader) {
      ...
      body.down('.drag-file-label').setHtml(
        Locale.t("pak.forms.documento.btn.tagattach.notsupported")
      );
  ...
  onDragEnter: function () {
    const b = this.getView().down("#v1-pak-dragattach");
  ...
  onDrop: function (target, info) {
    let me = this,
      ...
      panelDrop = view.down("#v1-pak-dragattach"),
      tagAttach = view.down('v1-pak-tagattach-combo'),
      files, len, s;
  ...
  onBuildPdf: function () {
    ...
    Ext.Ajax.request({
      ...
      success: function () {
        Ext.toast({
          html: Locale.t("pak.forms.documento.buildpdf.avvio"),
          closable: false,
          slideInDuration: 400,
          minWidth: 400
        });
      ...
      failure: function () {
        Ext.toast({
          html: Locale.t("pak.forms.documento.buildpdf.errore"),
          closable: false,
          slideInDuration: 400,
          minWidth: 400
        });
  ...
  onDownloafSpoolOriginPdf: function () {
    ...
    Ext.Ajax.request({
      ...
      success: function (response) {
        ...
        window.URL.revokeObjectURL(binaryFile);
        Ext.toast({
          html: Locale.t("pak.forms.documento.buildpdf.avviodownload"),
          closable: false,
          slideInDuration: 400,
          minWidth: 400
        });
      ...
      failure: function () {
        Ext.toast({
          html: Locale.t("pak.forms.documento.buildpdf.erroredownload"),
          closable: false,
          slideInDuration: 400,
          minWidth: 400
        });
  ...
  runDupOpen: function (record) {
    ...
    let newTab = {
      ...
      text: Locale.t("pak.forms.documento.infodoc"),
      btn: {
        xtype: "splitbutton",
