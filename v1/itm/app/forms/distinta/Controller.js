 ...
 */
Ext.define('itm.forms.distinta.Controller', {
  extend: 'portal.v1.view.forms.mainCard.Controller',
  mixins: [
    'portal.v1.global.Util',
    'itm.forms.articoli.mixins.Renaudo',
    'itm.forms.articoli.mixins.Telebi',
    'itm.forms.distinta.controller.ManagerCards',
    'itm.forms.distinta.controller.ManagerBtn',
    'itm.forms.articolo.controller.ControllerParts',
    'itm.forms.distinta.controller.ControllerDashboard',
  ],
  alias: 'controller.v1-itm-form-distinta',
  requires: [
    ...
    'Ext.util.Format',
    'Ext.window.Window',
    'itm.model.forms.distinta.Model',
    // 'itm.forms.distinta.component.gridAttributi.ModelAttributi',
    // 'itm.forms.distinta.component.gridlegami.GridLegami',
    // 'itm.forms.distinta.component.comboArticoli.storeArticolo',
    // 'itm.forms.distinta.cards.GridAttributi',
    'Ext.button.Split'
  ],
  init: function () {
    ...
    this.btnGetattributi = {
      ...
      ui: 'blue',
      text: Locale.t('itm.forms.distinta.btn.getattributi.text') + '...',
      tooltip: Locale.t('itm.forms.distinta.btn.getattributi.tooltip'),
      iconCls: 'x-fas fa-clone',
      handler: 'onGetAttributi'
    };
    vm.set('isnew', this.getView().valori.isnew);
    vm.set('id', this.getView().valori.id);
    vm.set('record', Ext.create('itm.model.forms.distinta.Model', {
  ...
  managerView: function (origin_record) {
    ...
    try {
      ...
      //titolo tab
      //TODO + ' - ' + Locale.t('itm.forms.distinta.fields.dateupdate') + ': ' + Ext.util.Format.date(record.data.dateupdate, 'd/m/Y H:i:s') || 'n.d.'
      vm.set('title', `<b>${record.data['cd_art']}</b> <i>${record.data['descrizione']}</i>`)
      vm.set('label', Locale.t('itm.forms.distinta.title'))
    ...
    storeattributi.each(function (rec) {
      if (rec.data.attributo !== '') {
        ...
        if (rec.data.valore.trim()==='') {
            err+= Locale.t('itm.forms.distinta.gridattributi.errvalore')+rec.data.attributo+'<br>'
        }
         */
      }
    })
  ...
  onClose: function (btn) {
    ...
    if (msgAlert) {
      Ext.Msg.show({ //chiedo conferma x eseguire la cancellazione
        title: Locale.t('itm.forms.distinta.btn.close.chiudi'), iconCls: 'x-fas fa-trash', msg: Locale.t('itm.forms.distinta.btn.close.chiuditext'),
  ...
  alertPredefinitoDel: function (delPre, listImg) {
    ...
    let gridPanel = Ext.create('Ext.grid.Panel', {
      ...
      columns: [
        {
          text: Locale.t('itm.forms.distinta.cards.docs.columns.predefinito'),
          xtype: 'widgetcolumn',
          widget: {
            height: 140,
            xtype: 'button',
        ...
        {
          text: Locale.t('itm.forms.distinta.cards.docs.columns.immagine'),
          width: 148,
          resizable: false,
          xtype: 'widgetcolumn',
          dataIndex: 'id',
        ...
        {
          text: Locale.t('itm.forms.distinta.cards.docs.columns.dettaglio'),
          xtype: 'widgetcolumn',
          flex: 1,
          widget: {
            xtype: 'component',
    ...
    let wndw = Ext.create('Ext.Window', {
      tbar: [btnX, btnConfirm], title: Locale.t('itm.forms.distinta.cards.docs.titlepred'),
      ...
      items: [
        {
          xtype: 'panel',
          html: '<b>' + Locale.t('itm.forms.distinta.cards.docs.msgpred') + '</b>'
        },
        gridPanel
      ]
    });
