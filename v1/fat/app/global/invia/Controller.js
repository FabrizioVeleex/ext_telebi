 ...
 */
Ext.define("fat.global.invia.Controller", {
  ...
  onAfterRender: function () {
    ...
    Ext.Ajax.request({
      ...
      success: function (res) {
        ...
        if (rest.totalCount > 0)
          vm.set("htmlNumeroDest", "<div style='font-size:18px;color:green'>" + Locale.t("fat.forms.documento.sendmail.nDest.yes") + ": " + rest.totalCount + "</div>")
        else
          vm.set("htmlNumeroDest", "<div style='font-size:18px;color:red'>" + Locale.t("fat.forms.documento.sendmail.nDest.no") + "</div>")
    ...
    if (noSpool.length > 0) {
      let result = Locale.t("fat.forms.documento.sendmail.documentsselected") + ":" + (nRecordGood - noSpool.length) + '/' + nRecordGood;
      if (noSpool.length > 0) {
        vm.set("htmlNoSpool", `<span style="font-size:19px;">${result} </br><span style="color:red;"><hr>${Locale.t("fat.forms.documento.sendmail.result")}</span></span>`);
      } else {
