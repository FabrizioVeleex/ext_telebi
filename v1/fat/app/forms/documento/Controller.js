Ext.define("fat.forms.documento.controller.Controller", {
  ...
  requires: [
    "Ext.window.Toast",
    "fat.forms.documento.Model",
    "fat.forms.documento.controller.ManagerBtn",
    "fat.forms.documento.controller.ManagerCards",
    'Ext.drag.Target',
  ...
  init: function () {
    ...
    vm.set(
      "record",
      Ext.create("fat.forms.documento.Model", {
  ...
  managerView: function () {
    ...
    let me = this,
      ...
      managerBtn = Ext.create("fat.forms.documento.controller.ManagerBtn"),
      managerCards = Ext.create("fat.forms.documento.controller.ManagerCards");

    try {
      // Titolo tab
      vm.set("title", record.data["num_doc"] + " - " + record.data["rag_soc"] || "n.d.");
      vm.set("label", Locale.t("fat.forms.documento.title"));
  ...
  onScarta: function () {
    ...
    Ext.Msg.show({
      title: Locale.t('fat.form.documento.btn.scarta.text'),
      iconCls: 'x-fas fa-trash',
      message: Locale.t('fat.form.documento.btn.scarta.messaggio'),
  ...
  onDwnExcel: function () {
    ...
    Ext.Ajax.request({
      ...
      failure: function (response) {
        ...
        // let errore = Locale.t("fat.forms.documento.errore") + ": " + response.statusText;
  ...
  onSendNewEmail: function () {
    ...
    if (!mailto) {
      Ext.Msg.show({
        title: Locale.t("global.attenzione"),
        message: Locale.t("global.form.validation.form") + " " + Locale.t("fat.forms.documento.fields.mailto"),
        buttons: Ext.Msg.OK,
        icon: Ext.MessageBox.ERROR,
      });
      return false;
    }
    console.log(Backend.REST_API + "global/sendmail/")
    ...
    Ext.Ajax.request({
      ...
      success: function (res) {
        ...
        Ext.toast({
          html: Locale.t("fat.forms.documento.sendmail.successsend"),
          closable: false,
          align: 't',
          slideInDuration: 400,
          minWidth: 400
      ...
      failure: function () {
        ...
        Ext.Msg.show({
          title: Locale.t("global.attenzione"),
          message: Locale.t("fat.forms.documento.sendmail.errorsend"),
          buttons: Ext.Msg.OK,
          icon: Ext.MessageBox.ERROR,
        });
      }
  ...
  onDragEnter: function () {
    const b = this.getView().down("#v1-fat-dragattach");
  ...
  onDrop: function (target, info) {
    ...
    let me = this,
      ...
      panelDrop = view.down("#v1-fat-dragattach"),
      tagAttach = view.down('v1-fat-tagattach-combo'),
      files, len, s;
  ...
  runDupOpen: function (record) {
    ...
    let newTab = {
      ...
      text: Locale.t("fat.forms.documento.infodoc"),
      btn: {
        xtype: "splitbutton",
