Ext.define("bol.forms.documento.controller.Controller", {
  ...
  requires: [
    "Ext.window.Toast",
    "bol.forms.documento.Model",
    "bol.forms.documento.controller.ManagerBtn",
    "bol.forms.documento.controller.ManagerCards",
    'Ext.drag.Target',
  ...
  init: function () {
    ...
    vm.set(
      "record",
      Ext.create("bol.forms.documento.Model", {
  ...
  managerView: function () {
    ...
    let me = this,
      ...
      managerBtn = Ext.create("bol.forms.documento.controller.ManagerBtn"),
      managerCards = Ext.create("bol.forms.documento.controller.ManagerCards");

    try {
      // Titolo tab
      vm.set("title", record.data["num_doc"] + " - " + record.data["rag_soc"] || "n.d.");
      vm.set("label", Locale.t("bol.forms.documento.title"));
  ...
  onScarta: function () {
    ...
    Ext.Msg.show({
      title: Locale.t('bol.form.documento.btn.scarta.text'),
      iconCls: 'x-fas fa-trash',
      message: Locale.t('bol.form.documento.btn.scarta.messaggio'),
  ...
  onSendNewEmail: function () {
    ...
    if (!mailto) {
      Ext.Msg.show({
        title: Locale.t("global.attenzione"),
        message: Locale.t("global.form.validation.form") + " " + Locale.t("bol.forms.documento.fields.mailto"),
        buttons: Ext.Msg.OK,
        icon: Ext.MessageBox.ERROR,
      });
      return false;
  ...
  afterRenderNewEmail: function (view) {
    ...
    if (window.File && window.FileList && window.FileReader) {
      ...
      body.down('.drag-file-label').setHtml(
        Locale.t("bol.forms.documento.btn.tagattach.notsupported")
      );
  ...
  onDragEnter: function () {
    const b = this.getView().down("#v1-bol-dragattach");
  ...
  onDrop: function (target, info) {
    let me = this,
      ...
      panelDrop = view.down("#v1-bol-dragattach"),
      tagAttach = view.down('v1-bol-tagattach-combo'),
      files, len, s;
  ...
  onBuildPdf: function () {
    ...
    Ext.Ajax.request({
      ...
      success: function () {
        Ext.toast({
          html: Locale.t("bol.forms.documento.buildpdf.avvio"),
          closable: false,
          align: 't',
          slideInDuration: 400,
          minWidth: 400
      ...
      failure: function () {
        Ext.toast({
          html: Locale.t("bol.forms.documento.buildpdf.errore"),
          closable: false,
          align: 't',
          slideInDuration: 400,
          minWidth: 400
  ...
  onDownloafSpoolOriginPdf: function () {
    ...
    Ext.Ajax.request({
      ...
      success: function (response) {
        ...
        window.URL.revokeObjectURL(binaryFile);
        Ext.toast({
          html: Locale.t("bol.forms.documento.buildpdf.avviodownload"),
          closable: false,
          align: 't',
          slideInDuration: 400,
          minWidth: 400
      ...
      failure: function () {
        Ext.toast({
          html: Locale.t("bol.forms.documento.buildpdf.erroredownload"),
          closable: false,
          align: 't',
          slideInDuration: 400,
          minWidth: 400
  ...
  runDupOpen: function (record) {
    ...
    let newTab = {
      ...
      text: Locale.t("bol.forms.documento.infodoc"),
      btn: {
        xtype: "splitbutton",
