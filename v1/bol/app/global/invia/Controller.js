 ...
 */
Ext.define("bol.global.invia.Controller", {
  extend: "Ext.app.ViewController",
  alias: "controller.bol-v1-global-invia",
  ...
  onAfterRender: function () {
    ...
    Ext.Ajax.request({
      ...
      success: function (res) {
        ...
        if (rest.totalCount !== 0)
          vm.set("htmlNumeroDest", "<div style='font-size:18px;color:green'>" + Locale.t("bol.forms.documento.sendmail.nDest.yes") + ": " + rest.totalCount + "</div>")
        else
          vm.set("htmlNumeroDest", "<div style='font-size:18px;color:red'>" + Locale.t("bol.forms.documento.sendmail.nDest.no") + "</div>")
      ...
      failure: function (res) {
        ...
        Ext.Msg.show({
          ...
          buttonText: {
            no: Locale.t("bol.grids.documenti.winprint.firmaok"),
          },
        });
      },
    });
    ...
    if (noSpool.length > 0) {
      let result = Locale.t("bol.forms.documento.sendmail.documentsselected") + ":" + (nRecordGood - noSpool.length) + '/' + nRecordGood;
      if (noSpool.length > 0) {
        vm.set("htmlNoSpool", `<span style="font-size:19px;">${result} </br><span style="color:red;"><hr>${Locale.t("bol.forms.documento.sendmail.result")}</span></span>`);
      } else {
