 ...
 */
Ext.define("bol.global.component.griddocumenti.GridDocumentiInviaError", {
  ...
  columns: [
      ...
      items: [
        {
          getClass: function (view, metadata, record) {
            if (record.get('spool') === 0) {
              metadata.tdAttr = `data-qtip="${Locale.t('bol.forms.documento.sendmail.columns.spool.no')}"`;
              return 'x-fas fa-times bd-action-null bd-color-red';
            }
            metadata.tdAttr = `data-qtip="${Locale.t('bol.forms.documento.sendmail.columns.spool.yes')}"`;
    ...
    {
      text: Locale.t('bol.forms.documento.sendmail.columns.totmail'),
      dataIndex: "totemail",
      width: 90,
    ...
    {
      text: Locale.t('bol.forms.documento.sendmail.columns.data_doc'),
      dataIndex: "data_doc",
      width: 120,
      xtype: "datecolumn",
      format: "d/m/Y",
    ...
    {
      text: Locale.t('bol.forms.documento.sendmail.columns.cd_ord'),
      dataIndex: "num_doc",
      width: 90,
      filter: { type: "string" },
    },
    {
      text: Locale.t('bol.forms.documento.sendmail.columns.cd_sogg_fat'),
